...
(function (global) {
  ...
  // stop-words PT pequenas e comuns
  var STOP = (function () {
    var w = ['de','da','do','das','dos','a','o','as','os','e','ou','um','uma','uns','umas',
             'em','no','na','nos','nas','para','por','com','sem','que','se','sua','seu','suas','seus',
             'meu','minha','meus','minhas','ao','aos','as','la','lo','quero','queria','tem','ter','tenho',
             'eh','é','isto','isso','aqui','ali','la','que','qual','quais','como','onde','quando','sobre'];
  ...
  async function ensureReady(progressCb) {
    ...
    state.loadingPromise = (async function () {
      progressCb && progressCb({ stage: 'data', progress: 0.05, message: 'Baixando catálogo...' });
      ...
      progressCb && progressCb({ stage: 'model', progress: 0.1, message: 'Carregando IA (~30 MB na 1ª vez, depois fica em cache)...' });
      ...
      state.extractor = await mod.pipeline('feature-extraction', MODEL_ID, {
        ...
        progress_callback: function (p) {
          ...
          if (p.status === 'progress' && typeof p.progress === 'number') {
            ...
            avg = n ? avg / n : 0;
            progressCb({ stage: 'model', progress: 0.1 + 0.85 * (avg / 100), message: 'Baixando IA... ' + Math.round(avg) + '%' });
          } else if (p.status === 'done' || p.status === 'ready') {
            progressCb({ stage: 'model', progress: 0.97, message: 'Quase lá...' });
          }
        }
      });

      state.ready = true;
      progressCb && progressCb({ stage: 'ready', progress: 1, message: 'IA pronta.' });
  ...
  async function searchAI(query) {
    if (!state.extractor) throw new Error('Modelo nao carregado. Chame ensureReady().');
    var q = (query || '').trim();
    if (!q) return null;
