...
(function (global) {
  ...
  function abTest(o) {
    ...
    if (!(nA > 0) || !(nB > 0)) return { error: 'O número de visitantes deve ser maior que zero.' };
    if (xA < 0 || xB < 0) return { error: 'O número de conversões não pode ser negativo.' };
    if (xA > nA || xB > nB) return { error: 'As conversões não podem superar o número de visitantes.' };

    var pA = xA / nA, pB = xB / nB;
    var pooled = (xA + xB) / (nA + nB);
    var sePooled = Math.sqrt(pooled * (1 - pooled) * (1 / nA + 1 / nB));
    if (sePooled === 0) {
      return { error: 'Sem variação nos dados (0% ou 100% nos dois grupos): o teste não se aplica.' };
    }
  ...
  function abSampleSize(o) {
    ...
    if (!(p1 > 0) || !(p1 < 1) || !(p2 > 0) || !(p2 < 1)) {
      return { error: 'As taxas de conversão devem estar entre 0% e 100%.' };
    }
    if (p1 === p2) return { error: 'A taxa esperada precisa ser diferente da taxa atual.' };
    var alpha = 1 - confidence;
  ...
  function tTest(o) {
    ...
    if (o.mode === 'one') {
      if (!(o.n >= 2)) return { error: 'A amostra precisa ter pelo menos 2 observações.' };
      if (!(o.sd > 0)) return { error: 'O desvio padrão precisa ser maior que zero.' };
      se = o.sd / Math.sqrt(o.n);
      t = (o.mean - mu0) / se;
      df = o.n - 1;
      estimate = o.mean; estimateLabel = 'media';
    } else if (o.mode === 'paired') {
      if (!(o.n >= 2)) return { error: 'A amostra precisa ter pelo menos 2 pares.' };
      if (!(o.sdDiff > 0)) return { error: 'O desvio padrão das diferenças precisa ser maior que zero.' };
      se = o.sdDiff / Math.sqrt(o.n);
      t = (o.meanDiff - mu0) / se;
      df = o.n - 1;
      estimate = o.meanDiff; estimateLabel = 'diferenca';
    } else { // 'two' — teste de Welch (não assume variâncias iguais)
      if (!(o.n1 >= 2) || !(o.n2 >= 2)) return { error: 'Cada amostra precisa ter pelo menos 2 observações.' };
      if (!(o.sd1 > 0) || !(o.sd2 > 0)) return { error: 'Os desvios padrão precisam ser maiores que zero.' };
  ...
  function zTest(o) {
    if (!(o.n >= 1)) return { error: 'O tamanho da amostra precisa ser pelo menos 1.' };
    if (!(o.sigma > 0)) return { error: 'O desvio padrão populacional precisa ser maior que zero.' };
    var tails = o.tails || 2, confidence = o.confidence, mu0 = o.mu0 || 0;
  ...
  function oneProportionTest(o) {
    if (!(o.n > 0)) return { error: 'O tamanho da amostra precisa ser maior que zero.' };
    if (o.x < 0 || o.x > o.n) return { error: 'O número de sucessos precisa estar entre 0 e n.' };
    if (!(o.p0 > 0) || !(o.p0 < 1)) return { error: 'A proporção hipotética deve estar entre 0% e 100%.' };
  ...
  function confidenceInterval(o) {
    ...
    if (o.mode === 'proportion') {
      if (!(o.n > 0)) return { error: 'O tamanho da amostra precisa ser maior que zero.' };
      if (o.x < 0 || o.x > o.n) return { error: 'Os sucessos precisam estar entre 0 e n.' };
      var ci = wilsonInterval(o.x, o.n, confidence);
      var phat = o.x / o.n;
      return {
  ...
  function surveySampleSize(o) {
    ...
    if (!(E > 0) || !(E < 1)) return { error: 'A margem de erro deve estar entre 0% e 100%.' };
    if (!(p >= 0) || !(p <= 1)) return { error: 'A proporção esperada deve estar entre 0% e 100%.' };
    var z = zCritical(confidence);
  ...
  function chiSquareTest(o) {
    ...
    var R = table.length;
    if (R < 2) return { error: 'A tabela precisa de pelo menos 2 linhas.' };
    var C = table[0].length;
    if (C < 2) return { error: 'A tabela precisa de pelo menos 2 colunas.' };
    var rowT = [], colT = [], grand = 0;
    for (var i = 0; i < R; i++) {
      if (table[i].length !== C) return { error: 'Todas as linhas precisam ter o mesmo número de colunas.' };
      rowT[i] = 0;
      for (var j = 0; j < C; j++) {
        var v = table[i][j];
        if (!isFinite(v) || v < 0) return { error: 'Preencha todas as células com números não negativos.' };
        rowT[i] += v;
        colT[j] = (colT[j] || 0) + v;
        grand += v;
      }
    }
    if (grand <= 0) return { error: 'A tabela não pode estar vazia.' };
    var chi2 = 0, minExpected = Infinity;
    for (var i = 0; i < R; i++) {
      for (var j = 0; j < C; j++) {
        var e = rowT[i] * colT[j] / grand;
        if (e === 0) return { error: 'Há uma linha ou coluna inteira com total zero.' };
  ...
  function anovaTest(o) {
    ...
    var k = groups.length;
    if (k < 2) return { error: 'Informe pelo menos 2 grupos.' };
    var N = 0, grandSum = 0, means = [], ns = [];
    for (var i = 0; i < k; i++) {
      var g = groups[i];
      if (g.length < 2) return { error: 'Cada grupo precisa de pelo menos 2 valores.' };
      var s = 0;
    ...
    var dfB = k - 1, dfW = N - k;
    if (dfW <= 0) return { error: 'Graus de liberdade insuficientes para o teste.' };
    var msB = ssB / dfB, msW = ssW / dfW;
    if (msW === 0) return { error: 'A variância dentro dos grupos é zero — o teste não se aplica.' };
    var F = msB / msW;
  ...
  function kruskalWallisTest(o) {
    ...
    var all = [], idx = [];
    for (var i = 0; i < k; i++) {
      if (groups[i].length < 1) return { error: 'Cada grupo precisa de pelo menos 1 valor.' };
      for (var j = 0; j < groups[i].length; j++) { all.push(groups[i][j]); idx.push(i); }
    }
    var N = all.length;
    if (N < 3) return { error: 'Informe mais observações para o teste.' };
  ...
  function mannWhitneyTest(o) {
    ...
    if (n1 < 1 || n2 < 1) return { error: 'Cada grupo precisa de pelo menos 1 valor.' };
    ...
    var sigmaU = Math.sqrt((n1 * n2 / 12) *
      ((N + 1) - rk.tieSum / (N * (N - 1))));
    if (!(sigmaU > 0)) return { error: 'Sem variação nos dados — o teste não se aplica.' };
    var z = (U - muU) / sigmaU;
  ...
  function wilcoxonSignedRankTest(o) {
    ...
    if (before.length !== after.length) {
      return { error: 'As duas colunas precisam ter o mesmo número de valores.' };
    }
    var diffs = [];
    ...
    var n = diffs.length;
    if (n < 1) return { error: 'Todas as diferenças são zero — não há o que testar.' };
    var absD = [];
    ...
    if (!(sigmaW > 0)) return { error: 'Sem variação nas diferenças — o teste não se aplica.' };
    var z = (W - muW) / sigmaW;
    var pValue = tails === 1 ? (1 - normalCDF(Math.abs(z)))
                             : 2 * (1 - normalCDF(Math.abs(z)));
    var alpha = 1 - confidence;
